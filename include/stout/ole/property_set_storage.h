...
STOUT_API auto write_property_set(storage &stg, std::string_view stream_name, const property_set &ps)
    -> std::expected<void, error>;

// ── Convenience: SummaryInformation ───────────────────────────────────

// Read the \005SummaryInformation property set from a storage.
[[nodiscard]] STOUT_API auto read_summary_info(storage &stg) -> std::expected<property_set, error>;

// Write the \005SummaryInformation property set to a storage.
STOUT_API auto write_summary_info(storage &stg, const property_set &ps) -> std::expected<void, error>;

// ── Convenience: DocumentSummaryInformation ───────────────────────────

// Read the \005DocumentSummaryInformation property set from a storage.
[[nodiscard]] STOUT_API auto read_doc_summary_info(storage &stg) -> std::expected<property_set, error>;

// Write the \005DocumentSummaryInformation property set to a storage.
STOUT_API auto write_doc_summary_info(storage &stg, const property_set &ps) -> std::expected<void, error>;

// ── Convenience builders ──────────────────────────────────────────────

// Create a minimal SummaryInformation property set with common fields.
[[nodiscard]] STOUT_API auto make_summary_info(std::string_view title = {}, std::string_view subject = {},
                                               std::string_view author = {}, std::string_view app_name = {})
    -> property_set;
