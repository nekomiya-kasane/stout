...
template <io::lock_bytes LB> class sector_io {
  public:
    explicit sector_io(LB &backend, uint32_t sector_size) noexcept : backend_(&backend), sector_size_(sector_size) {}
    ...
    auto read_at(uint32_t sector_id, uint32_t offset_in_sector, std::span<uint8_t> buf)
    ...
    auto write_at(uint32_t sector_id, uint32_t offset_in_sector, std::span<const uint8_t> buf)
        -> std::expected<size_t, error> {
        auto offset = sector_offset(sector_id, sector_size_) + offset_in_sector;
