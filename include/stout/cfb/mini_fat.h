...
namespace stout::cfb {
    ...
    class STOUT_API mini_stream_io {
      public:
        ...
        template <io::lock_bytes LB>
        auto read_mini_stream(sector_io<LB> &sio, const mini_fat_table &mfat, uint32_t start_mini_sector,
                              uint64_t offset, std::span<uint8_t> buf) -> std::expected<size_t, error> {
        ...
        template <io::lock_bytes LB>
        auto write_mini_stream(sector_io<LB> &sio, const mini_fat_table &mfat, uint32_t start_mini_sector,
                               uint64_t offset, std::span<const uint8_t> buf) -> std::expected<size_t, error> {
            auto chain = mfat.chain(start_mini_sector);
            if (chain.empty()) {
                return size_t{0};
            }
