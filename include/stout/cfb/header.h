...
struct cfb_header {
    ...
    [[nodiscard]] auto sector_size() const noexcept -> uint32_t {
        return 1u << sector_shift;
    }

    [[nodiscard]] auto mini_sector_size_val() const noexcept -> uint32_t {
        return 1u << mini_sec_shift;
    }
...
[[nodiscard]] STOUT_API auto validate_header(const cfb_header& hdr) noexcept
