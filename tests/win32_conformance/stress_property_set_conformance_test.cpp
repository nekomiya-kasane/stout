...
protected:
    ...
    static HRESULT open_summary_read(const std::wstring& path,
                                      propset_storage_ptr& pss,
                                      propset_ptr& ps) {
        storage_ptr stg;
        HRESULT hr = win32_open_read(path, stg.put());
        if (FAILED(hr)) return hr;
        hr = stg->QueryInterface(IID_IPropertySetStorage,
                                  reinterpret_cast<void**>(pss.put()));
        if (FAILED(hr)) return hr;
        hr = pss->Open(FMTID_SummaryInformation,
                        STGM_READ | STGM_SHARE_EXCLUSIVE, ps.put());
        return hr;
    }
};
...
TEST_P(StressPropertySetConformance, TitleStringStoutToWin32) {
    auto p = temp_file("sp_title"); guard_.add(p);
    {
    ...
    propset_storage_ptr pss; propset_ptr prop;
    ASSERT_TRUE(SUCCEEDED(open_summary_read(p.wstring(), pss, prop)));
    PROPSPEC spec{}; spec.ulKind = PRSPEC_PROPID; spec.propid = PIDSI_TITLE;
...
TEST_P(StressPropertySetConformance, AuthorStringStoutToWin32) {
    auto p = temp_file("sp_author"); guard_.add(p);
    {
        auto cf = compound_file::create(p, GetParam().ver);
        ASSERT_TRUE(cf.has_value());
        property_set ps;
    ...
    propset_storage_ptr pss; propset_ptr prop;
    ASSERT_TRUE(SUCCEEDED(open_summary_read(p.wstring(), pss, prop)));
    PROPSPEC spec{}; spec.ulKind = PRSPEC_PROPID; spec.propid = PIDSI_AUTHOR;
    PROPVARIANT var{}; PropVariantInit(&var);
    ASSERT_TRUE(SUCCEEDED(prop->ReadMultiple(1, &spec, &var)));
    EXPECT_EQ(var.vt, VT_LPSTR);
...
TEST_P(StressPropertySetConformance, SubjectStringStoutToWin32) {
    auto p = temp_file("sp_subj"); guard_.add(p);
    {
        auto cf = compound_file::create(p, GetParam().ver);
        ASSERT_TRUE(cf.has_value());
        property_set ps;
    ...
    propset_storage_ptr pss; propset_ptr prop;
    ASSERT_TRUE(SUCCEEDED(open_summary_read(p.wstring(), pss, prop)));
    PROPSPEC spec{}; spec.ulKind = PRSPEC_PROPID; spec.propid = PIDSI_SUBJECT;
    PROPVARIANT var{}; PropVariantInit(&var);
    ASSERT_TRUE(SUCCEEDED(prop->ReadMultiple(1, &spec, &var)));
    EXPECT_EQ(var.vt, VT_LPSTR);
...
TEST_P(StressPropertySetConformance, MultiplePropertiesStoutToWin32) {
    auto p = temp_file("sp_multi"); guard_.add(p);
    {
        auto cf = compound_file::create(p, GetParam().ver);
        ASSERT_TRUE(cf.has_value());
        property_set ps;
...
TEST_P(StressPropertySetConformance, PageCountIntStoutToWin32) {
    auto p = temp_file("sp_int"); guard_.add(p);
    {
        auto cf = compound_file::create(p, GetParam().ver);
        ASSERT_TRUE(cf.has_value());
        property_set ps;
    ...
    propset_storage_ptr pss; propset_ptr prop;
    ASSERT_TRUE(SUCCEEDED(open_summary_read(p.wstring(), pss, prop)));
    PROPSPEC spec{}; spec.ulKind = PRSPEC_PROPID; spec.propid = PIDSI_PAGECOUNT;
    PROPVARIANT var{}; PropVariantInit(&var);
    ASSERT_TRUE(SUCCEEDED(prop->ReadMultiple(1, &spec, &var)));
...
TEST_P(StressPropertySetConformance, TitleWin32ToStout) {
    auto p = temp_file("sp_w2s_title"); guard_.add(p);
    {
        storage_ptr stg;
        ...
        propset_storage_ptr pss;
        ASSERT_TRUE(SUCCEEDED(stg->QueryInterface(IID_IPropertySetStorage,
            reinterpret_cast<void**>(pss.put()))));
        propset_ptr ps;
        ASSERT_TRUE(SUCCEEDED(pss->Create(FMTID_SummaryInformation, nullptr,
            PROPSETFLAG_DEFAULT, STGM_CREATE | STGM_READWRITE | STGM_SHARE_EXCLUSIVE, ps.put())));
        PROPSPEC spec{}; spec.ulKind = PRSPEC_PROPID; spec.propid = PIDSI_TITLE;
        PROPVARIANT var{}; var.vt = VT_LPSTR; var.pszVal = const_cast<char*>("Win32 Title");
...
TEST_P(StressPropertySetConformance, IntWin32ToStout) {
    auto p = temp_file("sp_w2s_int"); guard_.add(p);
    {
        storage_ptr stg;
        if (GetParam().ver == cfb_version::v4) ASSERT_TRUE(SUCCEEDED(win32_create_v4(p.wstring(), stg.put())));
        else ASSERT_TRUE(SUCCEEDED(win32_create_v3(p.wstring(), stg.put())));
        ...
        ASSERT_TRUE(SUCCEEDED(pss->Create(FMTID_SummaryInformation, nullptr,
            PROPSETFLAG_DEFAULT, STGM_CREATE | STGM_READWRITE | STGM_SHARE_EXCLUSIVE, ps.put())));
        PROPSPEC spec{}; spec.ulKind = PRSPEC_PROPID; spec.propid = PIDSI_PAGECOUNT;
        PROPVARIANT var{}; var.vt = VT_I4; var.lVal = 99;
        ASSERT_TRUE(SUCCEEDED(ps->WriteMultiple(1, &spec, &var, 2)));
    }
    auto cf = compound_file::open(p, open_mode::read);
    ASSERT_TRUE(cf.has_value());
...
TEST_P(StressPropertySetConformance, RoundtripModify) {
    auto p = temp_file("sp_rt"); guard_.add(p);
    {
        auto cf = compound_file::create(p, GetParam().ver);
        ASSERT_TRUE(cf.has_value());
        property_set ps;
    ...
    }
    {
        ...
        ASSERT_TRUE(SUCCEEDED(stg->QueryInterface(IID_IPropertySetStorage,
            reinterpret_cast<void**>(pss.put()))));
        propset_ptr ps;
        ASSERT_TRUE(SUCCEEDED(pss->Open(FMTID_SummaryInformation,
            STGM_READWRITE | STGM_SHARE_EXCLUSIVE, ps.put())));
        PROPSPEC spec{}; spec.ulKind = PRSPEC_PROPID; spec.propid = PIDSI_TITLE;
        PROPVARIANT var{}; var.vt = VT_LPSTR; var.pszVal = const_cast<char*>("Modified");
        ASSERT_TRUE(SUCCEEDED(ps->WriteMultiple(1, &spec, &var, 2)));
    }
    auto cf = compound_file::open(p, open_mode::read);
    ASSERT_TRUE(cf.has_value());
...
TEST_P(StressPropertySetConformance, EmptyStringProperty) {
    auto p = temp_file("sp_empty"); guard_.add(p);
    {
        auto cf = compound_file::create(p, GetParam().ver);
        ASSERT_TRUE(cf.has_value());
        property_set ps;
...
TEST_P(StressPropertySetConformance, LongStringProperty) {
    auto p = temp_file("sp_long"); guard_.add(p);
...
TEST_P(StressPropertySetConformance, NoPropertySetReturnsError) {
    auto p = temp_file("sp_none"); guard_.add(p);
...
TEST_P(StressPropertySetConformance, SecurityPropertyStoutToWin32) {
    auto p = temp_file("sp_sec"); guard_.add(p);
    {
        auto cf = compound_file::create(p, GetParam().ver);
        ASSERT_TRUE(cf.has_value());
        property_set ps;
    ...
    propset_storage_ptr pss; propset_ptr prop;
    ASSERT_TRUE(SUCCEEDED(open_summary_read(p.wstring(), pss, prop)));
    PROPSPEC spec{}; spec.ulKind = PRSPEC_PROPID; spec.propid = PIDSI_DOC_SECURITY;
    PROPVARIANT var{}; PropVariantInit(&var);
    ASSERT_TRUE(SUCCEEDED(prop->ReadMultiple(1, &spec, &var)));
    EXPECT_EQ(var.vt, VT_I4);
...
TEST_P(StressPropertySetConformance, KeywordsStoutToWin32) {
    auto p = temp_file("sp_kw"); guard_.add(p);
    {
        auto cf = compound_file::create(p, GetParam().ver);
        ASSERT_TRUE(cf.has_value());
        property_set ps;
    ...
    propset_storage_ptr pss; propset_ptr prop;
    ASSERT_TRUE(SUCCEEDED(open_summary_read(p.wstring(), pss, prop)));
    PROPSPEC spec{}; spec.ulKind = PRSPEC_PROPID; spec.propid = PIDSI_KEYWORDS;
    PROPVARIANT var{}; PropVariantInit(&var);
    ASSERT_TRUE(SUCCEEDED(prop->ReadMultiple(1, &spec, &var)));
    EXPECT_EQ(var.vt, VT_LPSTR);
