...
TEST_P(StressMultiStreamConformance, TwentyVaryingSizes) {
    auto p = temp_file("ms_20var"); guard_.add(p);
    {
...
TEST_P(StressMultiStreamConformance, TwentySameSize) {
    auto p = temp_file("ms_20same"); guard_.add(p);
    {
        auto cf = compound_file::create(p, GetParam().ver);
        ASSERT_TRUE(cf.has_value());
        for (int i = 0; i < 20; ++i) {
...
TEST_P(StressMultiStreamConformance, TwentyMiniStreams) {
    auto p = temp_file("ms_20mini"); guard_.add(p);
    {
        auto cf = compound_file::create(p, GetParam().ver);
        ASSERT_TRUE(cf.has_value());
        for (int i = 0; i < 20; ++i) {
...
TEST_P(StressMultiStreamConformance, FiveRegularStreams) {
    auto p = temp_file("ms_5reg"); guard_.add(p);
    {
        auto cf = compound_file::create(p, GetParam().ver);
        ASSERT_TRUE(cf.has_value());
        for (int i = 0; i < 5; ++i) {
...
TEST_P(StressMultiStreamConformance, MixedMiniRegular15) {
    auto p = temp_file("ms_mixed15"); guard_.add(p);
    {
        auto cf = compound_file::create(p, GetParam().ver);
        ASSERT_TRUE(cf.has_value());
        for (int i = 0; i < 15; ++i) {
...
TEST_P(StressMultiStreamConformance, StreamsInSubStorages) {
    auto p = temp_file("ms_substg"); guard_.add(p);
    {
        auto cf = compound_file::create(p, GetParam().ver);
        ASSERT_TRUE(cf.has_value());
        for (int i = 0; i < 5; ++i) {
...
TEST_P(StressMultiStreamConformance, DataIntegrity10Streams) {
    auto p = temp_file("ms_integ"); guard_.add(p);
    {
        auto cf = compound_file::create(p, GetParam().ver);
        ASSERT_TRUE(cf.has_value());
        for (int i = 0; i < 10; ++i) {
...
TEST_P(StressMultiStreamConformance, Win32Creates15StoutReads) {
    auto p = temp_file("ms_w32_15"); guard_.add(p);
    {
        storage_ptr stg;
...
TEST_P(StressMultiStreamConformance, EmptyAndDataMixed) {
    auto p = temp_file("ms_emptydata"); guard_.add(p);
    {
        auto cf = compound_file::create(p, GetParam().ver);
        ASSERT_TRUE(cf.has_value());
        for (int i = 0; i < 10; ++i) {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        ASSERT_TRUE(SUCCEEDED(stg->OpenStream(name.c_str(), nullptr,
            STGM_READ | STGM_SHARE_EXCLUSIVE, 0, strm.put())));
        if (i % 2 == 0) EXPECT_EQ(win32_stream_size(strm.get()), 500u);
        else EXPECT_EQ(win32_stream_size(strm.get()), 0u);
    }
}
...
TEST_P(StressMultiStreamConformance, StoragesAndStreamsAtRoot) {
    auto p = temp_file("ms_rootmix"); guard_.add(p);
    {
        auto cf = compound_file::create(p, GetParam().ver);
        ASSERT_TRUE(cf.has_value());
        for (int i = 0; i < 10; ++i) {
