...
TEST_P(StressCrossAPIConformance, Win32ComplexStoutReads) {
    auto p = temp_file("sx_w2s"); guard_.add(p);
    {
        storage_ptr stg;
        ...
        stream_ptr substrm;
        ASSERT_TRUE(SUCCEEDED(sub->CreateStream(L"SubStream",
            STGM_CREATE | STGM_READWRITE | STGM_SHARE_EXCLUSIVE, 0, 0, substrm.put())));
...
TEST_P(StressCrossAPIConformance, StoutComplexWin32Reads) {
    auto p = temp_file("sx_s2w"); guard_.add(p);
    {
...
TEST_P(StressCrossAPIConformance, RoundtripModify) {
    auto p = temp_file("sx_rt"); guard_.add(p);
...
TEST_P(StressCrossAPIConformance, RoundtripStoutModifies) {
    auto p = temp_file("sx_rt2"); guard_.add(p);
    ...
    for (auto& e : entries) free_statstg_name(e);
    stream_ptr strm;
    ASSERT_TRUE(SUCCEEDED(stg->OpenStream(L"Added", nullptr,
        STGM_READ | STGM_SHARE_EXCLUSIVE, 0, strm.put())));
...
TEST_P(StressCrossAPIConformance, Win32DeepHierarchyStoutTraverses) {
    auto p = temp_file("sx_deep"); guard_.add(p);
    {
        storage_ptr stg;
        if (GetParam().ver == cfb_version::v4) ASSERT_TRUE(SUCCEEDED(win32_create_v4(p.wstring(), stg.put())));
        else ASSERT_TRUE(SUCCEEDED(win32_create_v3(p.wstring(), stg.put())));
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            ASSERT_TRUE(SUCCEEDED(cur->CreateStorage(name.c_str(),
                STGM_CREATE | STGM_READWRITE | STGM_SHARE_EXCLUSIVE, 0, 0, levels[i].put())));
            cur = levels[i].get();
        }
        stream_ptr leaf;
        ASSERT_TRUE(SUCCEEDED(cur->CreateStream(L"Leaf",
            STGM_CREATE | STGM_READWRITE | STGM_SHARE_EXCLUSIVE, 0, 0, leaf.put())));
        auto d = make_test_data(50, 0xCC);
        ASSERT_TRUE(SUCCEEDED(win32_stream_write(leaf.get(), d.data(), 50)));
    }
    auto cf = compound_file::open(p, open_mode::read);
    ASSERT_TRUE(cf.has_value());
    auto s0 = cf->root_storage().open_storage("Level0"); ASSERT_TRUE(s0.has_value());
    auto s1 = s0->open_storage("Level1"); ASSERT_TRUE(s1.has_value());
    auto s2 = s1->open_storage("Level2"); ASSERT_TRUE(s2.has_value());
    auto s3 = s2->open_storage("Level3"); ASSERT_TRUE(s3.has_value());
    auto s4 = s3->open_storage("Level4"); ASSERT_TRUE(s4.has_value());
    auto leaf = s4->open_stream("Leaf"); ASSERT_TRUE(leaf.has_value());
...
TEST_P(StressCrossAPIConformance, MixedMiniRegularCrossAPI) {
    auto p = temp_file("sx_mixed"); guard_.add(p);
    {
        auto cf = compound_file::create(p, GetParam().ver);
        ASSERT_TRUE(cf.has_value());
        auto root = cf->root_storage();
...
TEST_P(StressCrossAPIConformance, EmptyFileCrossAPI) {
    auto p = temp_file("sx_empty"); guard_.add(p);
...
TEST_P(StressCrossAPIConformance, Win32EmptyStoutReads) {
    auto p = temp_file("sx_w32empty"); guard_.add(p);
    {
        storage_ptr stg;
        if (GetParam().ver == cfb_version::v4) ASSERT_TRUE(SUCCEEDED(win32_create_v4(p.wstring(), stg.put())));
        else ASSERT_TRUE(SUCCEEDED(win32_create_v3(p.wstring(), stg.put())));
