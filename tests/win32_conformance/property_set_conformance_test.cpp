...
TEST_F(PropertySetConformance, WriteMultipleProperties) {
    ...
    PROPSPEC specs[3]{};
    specs[0].ulKind = PRSPEC_PROPID; specs[0].propid = PIDSI_TITLE;
    specs[1].ulKind = PRSPEC_PROPID; specs[1].propid = PIDSI_AUTHOR;
    specs[2].ulKind = PRSPEC_PROPID; specs[2].propid = PIDSI_PAGECOUNT;
    PROPVARIANT vars[3]{};
...
TEST_F(PropertySetConformance, Win32WriteStoutRead) {
...

    {
        ...
        ASSERT_TRUE(SUCCEEDED(pss->Create(FMTID_SummaryInformation, nullptr,
            PROPSETFLAG_DEFAULT, STGM_CREATE | STGM_READWRITE | STGM_SHARE_EXCLUSIVE,
            prop.put())));
...
TEST_F(PropertySetConformance, RoundtripStoutWin32Stout) {
    ...
    // Win32 reads and modifies
    {
        ...
        ASSERT_TRUE(SUCCEEDED(pss->Open(FMTID_SummaryInformation,
            STGM_READWRITE | STGM_SHARE_EXCLUSIVE, prop.put())));
...
TEST_F(PropertySetConformance, SubjectAndKeywords) {
    ...
    PROPSPEC specs[2]{};
    specs[0].ulKind = PRSPEC_PROPID; specs[0].propid = PIDSI_SUBJECT;
    specs[1].ulKind = PRSPEC_PROPID; specs[1].propid = PIDSI_KEYWORDS;
    PROPVARIANT vars[2]{};
    for (auto& v : vars) PropVariantInit(&v);
