...
static void resize_and_verify(const std::filesystem::path& path, cfb_version ver,
                                size_t initial_sz, size_t new_sz, uint8_t seed) {
    ...
    stream_ptr strm;
    ASSERT_TRUE(SUCCEEDED(stg->OpenStream(L"R", nullptr,
...
TEST_P(StressResizeConformance, Mini2000ToReg5000) {
    auto p = temp_file("sr_m2k_r5k"); guard_.add(p);
...
TEST_P(StressResizeConformance, ResizeDoesNotAffectOthers) {
    auto p = temp_file("sr_others"); guard_.add(p);
    ...
    ASSERT_TRUE(SUCCEEDED(stg->OpenStream(L"B", nullptr,
        STGM_READ | STGM_SHARE_EXCLUSIVE, 0, sb.put())));
    ...
    // A should be 5000
    stream_ptr sa;
    ASSERT_TRUE(SUCCEEDED(stg->OpenStream(L"A", nullptr,
        STGM_READ | STGM_SHARE_EXCLUSIVE, 0, sa.put())));
...
TEST_P(StressResizeConformance, Win32CreateStoutResizeWin32Verify) {
    auto p = temp_file("sr_w32"); guard_.add(p);
    auto data = make_test_data(6000, 0x55);
    {
        ...
        stream_ptr strm;
        ASSERT_TRUE(SUCCEEDED(stg->CreateStream(L"R",
...
TEST_P(StressResizeConformance, ResizeToExactSectorSize) {
    auto p = temp_file("sr_sector"); guard_.add(p);
...
TEST_P(StressResizeConformance, ResizeToTwoSectors) {
    auto p = temp_file("sr_2sec"); guard_.add(p);
    auto ss = GetParam().ver == cfb_version::v4 ? 4096u : 512u;
