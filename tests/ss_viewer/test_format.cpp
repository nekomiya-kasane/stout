...
TEST(Format, SizeBytes) {
    EXPECT_EQ(format_size(0), "0 B");
}
TEST(Format, SizeSmall) {
    EXPECT_EQ(format_size(512), "512 B");
}
TEST(Format, SizeKB) {
    EXPECT_EQ(format_size(1024), "1.0 KB");
}
TEST(Format, SizeKBFraction) {
    EXPECT_EQ(format_size(1536), "1.5 KB");
}
TEST(Format, SizeMB) {
    EXPECT_EQ(format_size(1048576), "1.0 MB");
}
TEST(Format, SizeGB) {
    EXPECT_EQ(format_size(1073741824ULL), "1.00 GB");
}
TEST(Format, SizeBoundary1023) {
    EXPECT_EQ(format_size(1023), "1023 B");
}
...
TEST(Format, EntryTypeRoot) {
    EXPECT_EQ(entry_type_str(stout::entry_type::root), "Root Storage");
}
TEST(Format, EntryTypeStorage) {
    EXPECT_EQ(entry_type_str(stout::entry_type::storage), "Storage");
}
TEST(Format, EntryTypeStream) {
    EXPECT_EQ(entry_type_str(stout::entry_type::stream), "Stream");
}
TEST(Format, EntryTypeUnknown) {
    EXPECT_EQ(entry_type_str(stout::entry_type::unknown), "Unknown");
}
