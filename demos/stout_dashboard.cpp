...
static void show_header(console& con) {
    con.newline();
    con.print_widget(
        rule_builder(" stout vs Win32 — Feature & Performance Dashboard ")
...
static double bench_stout_enumerate(const fs::path& path, int iters) {
    ...
    // Warmup
    for (int i = 0; i < 5; ++i) { volatile auto kids = root.children(); (void)kids; }
    auto t0 = std::chrono::high_resolution_clock::now();
    for (int i = 0; i < iters; ++i) {
...
static double bench_win32_create(const fs::path& path, int iters) {
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        StgCreateDocfile(wpath.c_str(),
            STGM_CREATE | STGM_READWRITE | STGM_SHARE_EXCLUSIVE, 0, &pStg);
        if (pStg) { pStg->Commit(STGC_DEFAULT); pStg->Release(); }
    }
    auto t0 = std::chrono::high_resolution_clock::now();
    for (int i = 0; i < iters; ++i) {
        IStorage* pStg = nullptr;
...
static double bench_win32_create_stream(const fs::path& path, int iters) {
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        pStg->CreateStream(name,
            STGM_CREATE | STGM_READWRITE | STGM_SHARE_EXCLUSIVE, 0, 0, &pStrm);
...
static double bench_win32_write_1k(const fs::path& path, int iters) {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        ULONG w = 0; pStrm->Write(buf.data(), static_cast<ULONG>(buf.size()), &w);
    }
    auto t0 = std::chrono::high_resolution_clock::now();
    for (int i = 0; i < iters; ++i) {
        pStrm->Seek(zero, STREAM_SEEK_SET, nullptr);
...
static double bench_win32_read_1k(const fs::path& path, int iters) {
    ...
    for (int i = 0; i < 10; ++i) {
        pStrm->Seek(zero, STREAM_SEEK_SET, nullptr);
        ULONG r = 0; pStrm->Read(rbuf.data(), static_cast<ULONG>(rbuf.size()), &r);
    }
    auto t0 = std::chrono::high_resolution_clock::now();
    for (int i = 0; i < iters; ++i) {
        pStrm->Seek(zero, STREAM_SEEK_SET, nullptr);
...
static void show_performance(console& con) {
    ...
    results.push_back({"Create empty file",
        bench_stout_create(stout_path, N_CREATE),
        bench_win32_create(win32_path, N_CREATE)});

    results.push_back({"Create stream",
        bench_stout_create_stream(stout_path, N_STREAM),
        bench_win32_create_stream(win32_path, N_STREAM)});

    results.push_back({"Write 1KB (mini)",
        bench_stout_write_1k(stout_path, N_IO),
        bench_win32_write_1k(win32_path, N_IO)});

    results.push_back({"Write 8KB (regular)",
        bench_stout_write_8k(stout_path, N_IO),
        bench_win32_write_8k(win32_path, N_IO)});

    results.push_back({"Read 1KB (mini)",
        bench_stout_read_1k(stout_path, N_IO),
        bench_win32_read_1k(win32_path, N_IO)});

    results.push_back({"Read 8KB (regular)",
        bench_stout_read_8k(stout_path, N_IO),
        bench_win32_read_8k(win32_path, N_IO)});

    results.push_back({"Enumerate 20 children",
        bench_stout_enumerate(stout_path, N_ENUM),
        bench_win32_enumerate(win32_path, N_ENUM)});

    results.push_back({"Write property set",
        bench_stout_property_set(stout_path, N_PROP),
...
static void show_code_comparison(console& con) {
    ...
    struct loc_entry { const char* name; int stout; int win32; };
    loc_entry locs[] = {
        {"Create + write stream",   4,  16},
        {"Read stream data",        3,  12},
        {"Enumerate children",      1,   8},
        {"Rename entry",            1,   3},
        {"Delete entry",            1,   3},
        {"Set timestamps",          1,   5},
        {"Property set write",      3,  25},
        {"Error handling",          1,   6},
    };
