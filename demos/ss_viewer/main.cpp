...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--win32") st.use_win32 = true;
        else if (arg == "--dump") dump_mode = true;
        else if (arg == "--dump-canvas") dump_canvas_mode = true;
        else if (arg == "--width" && i + 1 < argc) dump_width = static_cast<uint32_t>(std::atoi(argv[++i]));
        else if (arg == "--height" && i + 1 < argc) dump_height = static_cast<uint32_t>(std::atoi(argv[++i]));
        else st.file_path = arg;
    }
    ...
    if (st.use_win32) {
        ...
        HRESULT hr = StgOpenStorage(wpath.c_str(), nullptr,
            STGM_READ | STGM_SHARE_DENY_WRITE, nullptr, 0, &st.root_stg);
        if (FAILED(hr)) {
    ...
    if (dump_canvas_mode) {
        ...
        std::fprintf(stderr, "=== DUMP-CANVAS: %ux%u (viewport_h=%d) canvas=%ux%u ===\n",
            dump_width, dump_height, viewport_h, wc.width, wc.height);
        wc.dump(std::cout);
    ...
    app.set_status([&](status_bar_builder& sb) {
        ssv::build_status(st, sb);
    });

    app.on_menu_action([&](int /*menu_idx*/, int /*item_idx*/, const std::string& label) {
        ssv::handle_menu(label, st, app);
    });

    app.on_key([&](const key_event& ke) -> bool {
        return ssv::handle_key(ke, st, app);
    });
