...
namespace ssv {
    ...
    [[nodiscard]] inline tapiru::rows_builder build_stats_tab(const entry_info &root) {
        ...
        rows.add(
            tapiru::text_builder(std::format("  Total stream data: [bold]{}[/]", format_size(s.total_stream_bytes))));
        rows.add(
            tapiru::text_builder(std::format("  Largest stream:    [bold]{}[/] ({})", format_size(s.largest_stream),
