    ...
    sized_box_builder build_content(viewer_state &st, int viewport_h) {
        ...
        if (st.prop_set && !st.prop_set->sections.empty()) {
            ...
            for (auto &section : st.prop_set->sections) {
                prop_rows.add(
                    text_builder(std::format("[bold bright_yellow]Section: {}[/]", format_guid(section.fmtid))));
    ...
    rows_builder build_frame(viewer_state &st, int viewport_h, const classic_app_theme &app_theme) {
        ...
        for (auto &m : menus) {
            bar += "[rgb(" + std::to_string(app_theme.menu_bar_bg.fg.r) + "," +
                   std::to_string(app_theme.menu_bar_bg.fg.g) + "," + std::to_string(app_theme.menu_bar_bg.fg.b) +
                   ") on_rgb(" + std::to_string(app_theme.menu_bar_bg.bg.r) + "," +
                   std::to_string(app_theme.menu_bar_bg.bg.g) + "," + std::to_string(app_theme.menu_bar_bg.bg.b) +
                   ")] " + m.label + " [/]";
        }
